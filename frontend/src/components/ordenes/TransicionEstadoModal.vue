<template>
  <BaseModal :show="show" z-index="z-[60]" @update:show="$emit('update:show', $event)" @close="handleClose">
    ...
    <div class="space-y-4">
      ...
      <!-- Budget approval required (RN-04) -->
      <div v-if="requierePresupuestoAprobado && !presupuestoAprobado" class="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
        <div class="flex">
          <div class="flex-shrink-0">
          ...
          <div class="ml-3">
            <p class="text-sm font-medium text-yellow-800">
              Regla de Negocio RN-04: Presupuesto requerido
            </p>
            <p class="mt-1 text-sm text-yellow-700">
              No se puede iniciar la reparación sin que el presupuesto esté aprobado por el cliente.
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="!estadoSaltado && !(requierePresupuestoAprobado && !presupuestoAprobado)" class="text-center py-4">
      ...
      <div v-if="mostrarCampoNotas && !estadoSaltado && !(requierePresupuestoAprobado && !presupuestoAprobado)">
...
const requierePresupuestoAprobado = computed(() => {
  return props.nuevoEstado === 'reparacion'
})
...
const botonDeshabilitado = computed(() => {
  return estadoSaltado.value || (requierePresupuestoAprobado.value && !props.presupuestoAprobado) || props.loading
})
