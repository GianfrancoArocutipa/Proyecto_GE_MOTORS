          ...
          <div class="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col h-full">
            ...
            <div class="p-6 flex-grow bg-gray-50/50">
              ...
              <div v-if="activeHistoryTab === 'diagnosticos'" class="space-y-4">
                ...
                <div v-else class="space-y-4">
                  <div v-for="d in diagnosticosFiltrados" :key="d.id" class="bg-white border border-gray-200 rounded-lg p-5 relative overflow-hidden hover:shadow-sm transition-shadow">
                    <div class="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-500"></div>
                    <div class="flex justify-between items-start mb-3">
                      <div>
                        <span class="font-bold text-gray-900 text-sm bg-gray-100 px-2 py-0.5 rounded tracking-wide mr-2">OT #{{ d.numero_ot || d.orden_id }}</span>
                        <span class="text-xs text-gray-500">{{ formatDate(d.created_at || d.fecha) }}</span>
                      </div>
                      <div class="text-xs text-gray-600">
                        <span class="text-gray-400">Por:</span> <span class="font-medium text-gray-800">{{ d.mecanico_nombre }}</span>
                      </div>
                    </div>
                    
                    <div class="mb-3">
                      <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Códigos de Falla</div>
                      <div class="flex flex-wrap gap-2">
                        <span 
                          v-for="(code, idx) in normalizeCodigos(d.codigos_falla)" 
                          :key="idx"
                          class="px-2.5 py-1 bg-red-50 border border-red-100 text-red-700 rounded text-xs font-mono font-bold"
                        >
                          {{ code }}
                        </span>
                        <span v-if="normalizeCodigos(d.codigos_falla).length === 0" class="text-xs text-green-600 bg-green-50 px-2.5 py-1 rounded border border-green-100 font-medium">
                    ...
                    <div class="bg-gray-50 border border-gray-100 p-3 rounded-md">
                      <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Observaciones</div>
                      <p v-if="d.observaciones && d.observaciones !== 'Sin observaciones'" class="text-sm text-gray-800 italic">"{{ d.observaciones }}"</p>
                      <p v-else class="text-sm text-gray-400 italic">Sin observaciones del mecánico</p>
                    </div>
                  </div>
                </div>
              </div>
